...
const sendOTPEmail = async (email, otp) => {
    ...
    try {
        ...
        message.subject     = 'CareBank - Your Verification Code';
        message.htmlContent = html;
        message.sender      = { name: 'CareBank', email: process.env.BREVO_SENDER_EMAIL };
        message.to          = [{ email }];

        const result = await emailAPI.sendTransacEmail(message);
        console.log(`✅ Email sent to ${email}: ${result.body?.messageId}`);
        return true;
